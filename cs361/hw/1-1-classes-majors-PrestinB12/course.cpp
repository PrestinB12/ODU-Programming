...
Course::Course()	//why no pass test?
: 	name(CourseName()), numberOfPrerequisites(0), maxPrerequisites(10),
		prereqs(new CourseName[maxPrerequisites])
{
	//stuff
}
/*
Course::Course(const Course& cname)
:name(CourseName()), numberOfPrerequisites(0), maxPrerequisites(10),
	prereqs(new CourseName[maxPrerequisites])
	{
		for (int x = 0; x < numberOfPrerequisites; ++x)
		{
			prereqs[x] = cname.prereqs[x];
		}
	}
*/
/**
...
bool Course::operator < (const Course &newCourse) const
{
	if (!(name==newCourse.name))	//beware if-else statement gore
	{
		return name < newCourse.name;
	}
	else if (numberOfPrerequisites != newCourse.numberOfPrerequisites)
	{
		return numberOfPrerequisites < newCourse.numberOfPrerequisites;
	}
	else if (maxPrerequisites != newCourse.maxPrerequisites)
	{
		return maxPrerequisites < newCourse.maxPrerequisites;
	}
	else
	{
		return *prereqs < *newCourse.prereqs;
	}
}

bool Course::operator == (const Course &newCourse) const
{
	if (!(name == newCourse.name) || numberOfPrerequisites != newCourse.numberOfPrerequisites || maxPrerequisites != newCourse.maxPrerequisites)
	{
		return false;
	}
	for (int x = 0; x < numberOfPrerequisites; x++)
	{
		if (!(prereqs[x] == newCourse.prereqs[x]))
		{
			return false;
		}
	}
...
const Course& Course::operator= (const Course& cname)
{
	if(this != &cname)
	{
		name = cname.name;
		numberOfPrerequisites = cname.numberOfPrerequisites;
		maxPrerequisites = cname.maxPrerequisites;
		delete [] prereqs;
		prereqs = new CourseName[maxPrerequisites];
